...
import { Box, Check, ChevronDown, ChevronRight, MoreVertical, RotateCcw } from "lucide-react";
import { useEffect, useMemo, useState } from "react";

import { CodeEditor } from "@/components/ui/code-editor";
import { Input } from "@/components/ui/input";
import { useComputeWithScenario, useUpdateOverrides, useValidateOverride } from "@/lib/api/hooks";
import { useProjectStore } from "@/store/projectState";
import { useScenarioStore } from "@/store/scenarioState";
import { useQueryClient } from "@tanstack/react-query";
import { Code2, FunctionSquare, Loader2, Pencil } from "lucide-react";
...
function ParameterItem({
  ...
  } else if (scenarioComputedValues?.[scenarioId]?.[node.id]?.scenario_value !== undefined) {
  ...
  if (effectiveValue !== undefined && effectiveValue !== null && baselineValue !== undefined && baselineValue !== null) {
  ...
  const handleValidateFormula = async () => {
    if (!node.value_computed || node.value_computed === null || node.value_computed === undefined) {
    ...
    try {
      ...
      if (result.ok) {
        toast.success(`Résultat: ${result.result}${node.unit ? ` ${node.unit}` : ''}`);
      } else {
    ...
    <div className="relative">
      {!isEditing ? (
        <SidebarItem
          ...
          label={
            <div className="flex items-center gap-2">
              <span className="truncate select-none text-white dark:text-white font-medium" title={node.label}>
                {node.label}
              </span>
              {override?.mode === 'formula' && (
                <Code2 className="h-3 w-3 text-blue-400 dark:text-blue-300 shrink-0" />
              )}
            </div>
          }
          ...
          rightContent={
            <>
              {isModified && (
                  <button
                      onClick={handleReset}
                      className="p-0.5 rounded hover:bg-zinc-200 dark:hover:bg-zinc-800 text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors opacity-0 group-hover:opacity-100"
                      title="Réinitialiser"
                  >
              ...
              <div
                  className={`ml-2 shrink-0 text-xs font-mono flex items-center justify-end transition-all ${
                      ...
                      isModified ? "font-semibold text-zinc-900 dark:text-zinc-100" : "text-zinc-500 dark:text-zinc-400"
                  }`}
                  onDoubleClick={handleDoubleClick}
                  title={isActive ? "Double-cliquer pour modifier" : undefined}
              >
                {isModified
                  ? (override.mode === 'formula' ? 'ƒ(x)' : formatNumber(override.override_value))
              ...
              <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
                  <DropdownMenuTrigger
                    className="p-0.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded transition-colors opacity-0 group-hover:opacity-100 focus:opacity-100"
                  ...
                  <DropdownMenuContent align="end" className="w-48">
                    ...
                    >
                      <Pencil className="h-3.5 w-3.5 mr-2 text-zinc-500" />
                      Modifier la valeur
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={(e) => {
                    ...
                    >
                      <FunctionSquare className="h-3.5 w-3.5 mr-2 text-zinc-500" />
                      Fonction
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
      ...
      {isEditing && mode === 'value' && (
        <div className="flex items-center gap-2 animate-in slide-in-from-top-1 duration-200" onClick={(e) => e.stopPropagation()}>
            <Input
                ...
                className="h-7 flex-1 text-xs px-2 bg-white dark:bg-zinc-950 border-blue-500 ring-1 ring-blue-500/20"
      ...
      {isEditing && mode === 'formula' && (
        <div className="flex flex-col gap-1.5 animate-in slide-in-from-top-1 duration-200 bg-zinc-50 dark:bg-zinc-900/40 rounded p-2 border border-zinc-200 dark:border-zinc-800" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-1.5 justify-end">
                <button
                    ...
                    className="px-2 py-1 rounded text-[10px] bg-zinc-200 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-200 hover:bg-zinc-300 dark:hover:bg-zinc-600 transition-colors disabled:opacity-50"
                >
                    {isValidating ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Tester'}
                </button>
                <button
                    onClick={handleSave}
                ...
                >
                    {updateOverrides.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : 'OK'}
                </button>
                <button
                    onClick={() => setIsEditing(false)}
                    className="px-2 py-1 rounded text-[10px] text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors"
...
export function ScenarioItem({
  ...
  return (
    <div className={`flex flex-col rounded-lg transition-all border ${
        isActive
            ? "border-blue-500/50 dark:border-blue-500/40 shadow-[0_0_15px_-3px_rgba(59,130,246,0.2)]"
        ...
        >
            ...
            <div className={`flex items-center justify-center w-4 h-4 rounded border-2 transition-colors shrink-0 ${
                isActive
            ...
            {/* Title & Color (Middle) */}
            <div className={`flex-1 flex items-center gap-3 min-w-0 ${isActive ? 'text-zinc-900 dark:text-zinc-100 font-semibold' : 'text-zinc-700 dark:text-zinc-300'}`}>
            ...
            <button
                ...
                className={`p-0.5 rounded transition-colors shrink-0 ${isActive ? 'text-blue-600 dark:text-blue-400 hover:bg-blue-500/20' : 'text-zinc-500 dark:text-zinc-400 hover:bg-white/10'}`}
            >
        ...
        {isExpanded && (
            <div className="pl-5 pr-2 pb-2 space-y-1 animate-in slide-in-from-top-1 duration-200">
                ...
                {parameters.length === 0 && (
                    <div className={`text-xs italic px-2 ${isActive ? 'text-zinc-600 dark:text-zinc-400' : 'text-zinc-500 dark:text-zinc-500'}`}>Aucun paramètre</div>
                )}
            </div>
        )}
    </div>
