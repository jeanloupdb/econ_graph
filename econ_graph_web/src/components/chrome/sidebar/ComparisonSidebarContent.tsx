...
function ComparisonNodeItem({
  ...
  return (
    <SidebarItem
      icon={<Box className="h-3.5 w-3.5" style={{ color: palette.border }} />}
              ...
              <DropdownMenuTrigger
                className={`p-1 rounded transition-colors shrink-0 ${
                  isSelected
                    ...
                    : "hover:bg-zinc-400 dark:hover:bg-white/10"
                }`}
...
export function ComparisonSidebarContent({
  ...
  const { parameterDiffs, intermediateDiffs, resultDiffs } = useMemo(() => {
    if (!comparisonValues)
      return { parameterDiffs: [], intermediateDiffs: [], resultDiffs: [] };
    ...
    const sortFn = (a: any, b: any) =>
      Math.abs(b.pct_diff) - Math.abs(a.pct_diff);
  ...
  const totalDiffs =
    parameterDiffs.length + intermediateDiffs.length + resultDiffs.length;
      ...
      <div className="p-4 border-b border-zinc-400 dark:border-white/5">
        <div className="flex items-stretch gap-3">
          ...
          <div className="flex items-center justify-center w-12 shrink-0">
            <span className="text-xs font-bold text-zinc-700 dark:text-zinc-500 tracking-widest">
              VS
            </span>
          </div>
                ...
                <div className="max-h-[300px] overflow-y-auto space-y-0.5">
                  {allScenarios.map((scenario) => (
                    <button
                      key={scenario.id}
                      onClick={() => {
                        setComparisonScenarios(scenario.id, scenarioBId);
                        setOpenA(false);
                      }}
                      className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors text-left"
                    >
                      <div
                  ...
                  {allScenarios
                    ...
                    .map((scenario) => (
                      <button
                        key={scenario.id}
                        onClick={() => {
                          setComparisonScenarios(scenarioAId, scenario.id);
                          setOpenB(false);
                        }}
                        className="w-full flex items-center gap-2.5 px-2.5 py-2 rounded-md hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors text-left"
                      >
                        <div
                          className="w-2 h-2 rounded-full shrink-0"
                          style={{
                            backgroundColor: scenario.color || "#94a3b8",
                          }}
                        />
                        <span className="text-sm text-zinc-900 dark:text-zinc-100">
                          {scenario.name}
        ...
        {!canCompare ? (
          <div className="flex flex-col items-center justify-center h-full py-12 px-6 text-center">
            <p className="text-xs text-zinc-800 dark:text-zinc-400">
              Sélectionnez deux scénarios différents pour comparer
            </p>
          </div>
        ) : isComputing ? (
          ...
          <div className="flex flex-col items-center justify-center h-full py-12 px-6 text-center">
            ...
            <p className="text-[11px] text-zinc-700 dark:text-zinc-400 mt-1">
              Les scénarios produisent les mêmes résultats
            </p>
          </div>
        )}
      </div>
